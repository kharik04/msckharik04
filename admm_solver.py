...
class Sub_problem:
    ...
    def set_cpl(self, A_coup, b_coup, c):
        ...
        if l[1]>l[0]:
            ...
            self.A_coup[self.p_id] = my_hstack((  np.zeros( (h,l[0]) ), A_pq,  np.zeros( (h, l[1]-u[0]) ), np.zeros(A_qp.shape), np.zeros( (h, l[2]-u[1]) ), 0.5*R     ))
            self.A_coup[self.q_id] = my_hstack((  np.zeros( (h,l[0]) ), np.zeros(A_pq.shape),  np.zeros( (h, l[1]-u[0]) ), A_qp, np.zeros( (h, l[2]-u[1]) ), 0.5*R     ))
        self.A_coup[self.p_id] = self.A_coup[self.p_id]#csr_matrix(self.A_qp)
        self.A_coup[self.q_id] = self.A_coup[self.q_id]#csr_matrix(self.A_pq)
    
    def initialise_dual(self):
        self.dual = {}
        self.dual[self.p_id] = np.zeros(self.A_coup[self.p_id].shape[0])
        self.dual[self.q_id] = np.zeros(self.A_coup[self.q_id].shape[0])
        self.dual2 = {}
...
def admm_solve(problem_name, algo):
    ...
    if algo == 'naive':
        ...
        with Model() as M:
            ...
            t0 = time.time()
            M.solve()
            print('time for naive solution:' , time.time() - t0)
                ...
                with Model() as M:
                    ...
                    for q_id in problems:
                        if str(p_id)+','+str(q_id) in subsubproblems or str(q_id)+','+str(p_id) in subsubproblems: #problems coupled to p
                            ...
                            slack1 = M.variable(f's_{p_id},{q_id}',  tp.A_coup[p_id].shape[0]  , Domain.greaterThan(0.0))
                            Epq = Expr.add( Expr.mul(tp.A_coup[p_id], lam_p) , tp.A_coup[q_id]@lam_q - tp.b_cpl)
